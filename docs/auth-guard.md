...
} catch (\luochuan\JWTAuth\Exceptions\UserNotDefinedException $e) {
    // do something
}

```

If the user is not set, then a `luochuan\JWTAuth\Exceptions\UserNotDefinedException` will be thrown
